...
> This decision is about **formwork itself**, not about any project built from it. `/formwork-init`
> removes it when initialising a new repo.

## Context

formwork's scripts (`gates.mjs`, `ratchet.mjs`, `doctor.mjs`) are the harness every project runs.
...
2. **formwork must serve projects in any language.** A Python or Go project should not need a
...
clean clone. Rejected only because `tsc` becomes a devDependency, so *developing* formwork needs
...
- formwork gains contributors who did not write it
